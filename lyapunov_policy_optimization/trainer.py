...
For an example, see examples/inverted_pendulum/train_model.py
'''
import torch

class Trainer():
    def __init__(self, model, lr, optimizer, loss_fn, dt, circle_tuning_loss_fn=None, falsifier=None, loss_mode='true'):
        self.model = model
        self.lr = lr
    ...
    def train(self, X, x_0, alpha=0.7, epochs=1000, verbose=False, every_n_epochs=10, step_size=100, decay_rate=1.):
        ...
        for epoch in range(1, epochs+1):
            ...
            if self.loss_mode == 'true':
                ...
                # compute lie derivative using linearized solution
                # add dimension to f_approx when passing into 
                f_approx_linearized = self.f_value_linearized(X, u)
                L_V_linearized =  self.get_lie_derivative(X, V_candidate, f_approx_linearized)
                # compute lie derivative using finite difference methods
                L_V_finite_difference = self.get_approx_lie_derivative(V_candidate, V_candidate_prime).squeeze()
                # combine information to balance knowledge of linearized system and possibly true noisy dynamics from trajectory
                L_V = alpha * L_V_linearized + (1. - alpha) * L_V_finite_difference
